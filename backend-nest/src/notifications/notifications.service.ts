...
export class NotificationsService {
  ...
  async handleTicketPagado(payload: { ticketId: string; idUsuario: string }) {
    ...
    try {
      ...
      if (!usuario) {
        this.logger.warn(`Usuario ${payload.idUsuario} no encontrado.`);
        return;
      }
      ...
      // Enviar el correo con el QR incrustado
      ...
      this.logger.log(`Correo enviado exitosamente a ${usuario.email}`);
    } catch (error) {
      this.logger.error(
  ...
  /**
   * Construye el HTML del correo electrónico utilizando Handlebars.
   * Esto previene ataques XSS y asegura un reemplazo global de variables.
   */
  private buildEmailHtml(nombre: string, ticketId: string): string {
    ...
    return template({ nombre, ticketId });
  }
}
