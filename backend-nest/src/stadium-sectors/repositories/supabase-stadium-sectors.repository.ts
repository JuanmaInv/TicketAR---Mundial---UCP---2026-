...
export class SupabaseSectoresRepository implements ISectoresRepository {
  ...
  async obtenerSectoresPorPartido(
    ...
    const [sectoresData, partidoSectoresData] = await Promise.all([
      ...
      this.supabaseService.getClient().from('partido_sectores').select('*').eq('id_partido', idPartido)
    ]);

    if (sectoresData.error) throw new Error(`Error al obtener sectores de estadio: ${sectoresData.error.message}`);
    if (partidoSectoresData.error) throw new Error(`Error al obtener sectores del partido ${idPartido}: ${partidoSectoresData.error.message}`);
  ...
  async obtenerSectoresTodosLosPartidos(): Promise<{ idPartido: string; sectores: SectorPorPartido[] }[]> {
    ...
    if (sectoresData.error) throw new Error(`Error al obtener sectores de estadio: ${sectoresData.error.message}`);
    if (partidoSectoresData.error) throw new Error(`Error al obtener sectores de todos los partidos: ${partidoSectoresData.error.message}`);
